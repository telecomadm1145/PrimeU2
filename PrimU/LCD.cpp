...
constexpr KeyMapping mapping[] = {{"Apps", 2.5, 39.5, 14, 4.5, 0xB1},
      ...
      while (1) {
        ...
        if (event.type == frame_event) {
          ...
          if (SDL_LockTexture(lcd_texture, NULL, &pixels, &pitch) == 0) {
            ...
            for (int i = 0; i < 320 * 240; i++) {
              RGB555 pixel = *reinterpret_cast<RGB555 *>(
                  &((uint8_t *)this->buffer)[i * 4]);
          ...
          ImGui::Text("Memory: %d bytes", sMemoryManager->HeapAllocCount.load());
          ...
          if (srf && txt) {
            ...
            for (auto b : mapping) {
              ...
              if (keystat[i] ^ ImGui::IsItemActive()) {
                ...
                uimp.status =
                    keystat[i] ? UI_EVENT_TYPE_KEY : UI_EVENT_TYPE_KEY_UP;
                EnqueueEvent(uimp);
              }
              ++i;
            }
            ...
            if (is_hovered && ImGui::IsMouseClicked(ImGuiMouseButton_Left)) {
              ...
              TouchUpdate((uint16_t)(mouse_pos.x - lcd_pos.x),
                          (uint16_t)(mouse_pos.y - lcd_pos.y), 0,
            ...
            if (is_dragging) {
              ...
              if (ImGui::IsMouseReleased(ImGuiMouseButton_Left)) {
                is_dragging = false;
                TouchUpdate((uint16_t)tx, (uint16_t)ty, 0,
