...
template<int N>      struct Reg { static_assert(N >= 0 && N <= 4); };
...
template<typename Wrapper, typename WrapperFunc>
...
template<typename T, HandleDomain Domain>
class HandleTable {
public:
	...
	auto with(HandleType h, Fn&& fn) -> std::optional<std::invoke_result_t<Fn, T&>> {
		std::lock_guard lk(m_mtx);
	...
	template<typename Fn>
	auto with_and_erase(HandleType h, Fn&& fn) -> std::optional<std::invoke_result_t<Fn, T&>> {
		std::lock_guard lk(m_mtx);
		auto it = m_map.find(h.value);
		if (it == m_map.end() || !it->second) return std::nullopt;
	...
	size_t size() const { std::lock_guard lk(m_mtx); return m_map.size(); }

	template<typename Fn>
	void for_each(Fn&& fn) {
		std::lock_guard lk(m_mtx);
		for (auto& [id, obj] : m_map)
			if (obj) fn(HandleType{ id }, *obj);
	}

	void clear() { std::lock_guard lk(m_mtx); m_map.clear(); }
...
private:
	...
	uint32_t alloc_locked() {
		while (m_map.contains(m_next) || m_next == 0) ++m_next;
		return m_next++;
	}
...
template<typename Derived, HandleDomain Domain>
class HandleObject {
public:
	using HandleType = Handle<Domain>;
...
struct PositionToSrc {
	// N >= 4: 栈上，偏移 (N-4)*4
	using type = Stack<static_cast<int>((N - 4) * 4)>;
};
template<> struct PositionToSrc<0> { using type = Reg<0>; };
template<> struct PositionToSrc<1> { using type = Reg<1>; };
template<> struct PositionToSrc<2> { using type = Reg<2>; };
template<> struct PositionToSrc<3> { using type = Reg<3>; };

template<std::size_t N>
using PositionToSrc_t = typename PositionToSrc<N>::type;

// ----------------------------------------------------------------
// 8.2  C++ 类型 -> 默认描述符 (As<T, Src>)
//      指针类型 -> VPtr<PointeeType, Src>
//      其余类型 -> As<T, Src>
// ----------------------------------------------------------------
...
// 1. 定义核心逻辑 Helper，它只关心 Ret 和 Args...，不关心原来的 Callable 是什么类型
template<typename Ret, typename... Args>
struct AutoBindHelper {
private:
	// 为第 I 个参数生成描述符
	template<std::size_t I>
	using Desc = MakeDescriptor_t<
	...
	static uint32_t invoke_impl(
		SystemServiceArguments* a,
...
public:
	// 真正的包装入口
	template<auto Fn>
	static uint32_t thunk(SystemServiceArguments* a)
	{
		return invoke_impl<Fn>(
			a,
...
template<typename T>
struct AutoBind : AutoBind<decltype(&T::operator())> {};

// (E) 针对成员函数指针（常量版本，通常对应 Lambda 的 operator()）
// 通过这个特化，我们将 Lambda 转换为了 Ret 和 Args...，并桥接到 AutoBindHelper
template<typename ClassType, typename Ret, typename... Args>
struct AutoBind<Ret(ClassType::*)(Args...) const> : AutoBindHelper<Ret, Args...> {};
...
template<typename ClassType, typename Ret, typename... Args>
struct AutoBind<Ret(ClassType::*)(Args...) const noexcept> : AutoBindHelper<Ret, Args...> {};

template<typename ClassType, typename Ret, typename... Args>
struct AutoBind<Ret(ClassType::*)(Args...) noexcept> : AutoBindHelper<Ret, Args...> {};
...
template<auto Fn>
inline constexpr SvcHandler wrap_fn_f = &AutoBind<std::remove_pointer_t<decltype(Fn)>>::template thunk<Fn>;

// 如果 decltype(Fn) 是函数指针，remove_pointer_t 转为函数类型，命中 (A)
// 如果 decltype(Fn) 是 Lambda 类，命中 (D) -> (E)

#define EXPORT(name, func)                                          \
    inline constexpr SvcHandler name = wrap_fn_f<func>; 