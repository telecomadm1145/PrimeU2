...
public:
  ...
  ErrorCode StaticAlloc(VirtPtr addr, size_t size,
                        MemoryBlock **memoryBlock = nullptr, int prot = 7);
  ErrorCode StaticFree(VirtPtr addr);

  void WriteCookie(VirtPtr addr);

  // 动态分配：仅在预映射的 32MB 虚拟堆内做子分配（不触发新的
  // uc_mem_map/uc_mem_unmap）
...
public:
  const std::unordered_set<MemoryBlock *> &GetMemoryBlocks() const {
    return _blocks;
  }
...
private:
  ...
  inline bool HeapContains(VirtPtr a) const {
    if (!_dynamicHeapBlock)
      return false;
    VirtPtr base = _dynamicHeapBlock->GetVAddr();
    return (a >= base) && (a < base + _heapSize);
  }

  inline bool RangeOverlaps(VirtPtr a1, size_t s1, VirtPtr a2,
                            size_t s2) const {
    if (s1 == 0 || s2 == 0)
      return false;
