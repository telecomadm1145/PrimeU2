...
void Thread::SetEvent(Event *ev) {
  ...
  if (ev->manualReset) {
    ...
    while (!ev->waiters.empty()) {
      ...
      // 清理等待状态，使线程在下次调度可运行（CanRun 会看到 _waitingEvent ==
      // nullptr）
    ...
    if (!ev->waiters.empty()) {
      ...
      // 没有等待者，则把事件置为 signaled，下一次 WaitForEvent
      // 会立即返回并清除（auto）
      ev->signaled = true;
    }
  }
...
// WaitForEvent: 非阻塞地把线程置为等待状态（与 EnterCriticalSection
// 的风格一致） timeoutMillis: 毫秒； <0 表示无限等待； ==0
// 表示不阻塞（立即返回） 注意：函数返回后线程处于等待状态（除非事件已被
// signaled）——由 CanRun 控制何时实际继续运行
void Thread::WaitForEvent(Event *ev, int timeoutMillis) {
  ...
  // Non-blocking: 返回后线程处于等待状态；调度器会在 CanRun()
  // 中阻塞它，直到事件或超时或 Resume
}
...
void Thread::Resume() {
  ...
  // 如果线程正在等待事件或临界区，但现在被 Resume，CanRun()
  // 会重新评估它是否可运行
}
...
bool Thread::CanRun() {
  ...
  // 4) 如果正在等待事件，则检查超时或事件是否已被 Set（SetEvent 会在唤醒时把
  // _waitingEvent 置 nullptr）
