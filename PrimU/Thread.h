...
class ThreadState {
public:
  ThreadState(VirtPtr startPtr, VirtPtr stackPtr, uint32_t arg)
      : _r0(arg), _sp(stackPtr), _pc(startPtr) {
...
private:
  ...
  void *_args[16] = {&_r0, &_r1, &_r2,  &_r3,  &_r4,  &_r5, &_r6, &_r7,
                     &_r8, &_r9, &_r10, &_r11, &_r12, &_sp, &_lr, &_pc};
};
...
class Thread {
public:
  Thread(Thread **currentThread, VirtPtr start, uint32_t arg, uint8_t priority,
         size_t stackSize)
    ...
    printf("Thread [%i] -> %08X stack starts at %08X and ends at %08X\n", _id,
           start, stackPtrStart, _stackAddr);
  ...
  void
  WaitForEvent(Event *ev,
               int timeoutMillis); // timeout in ms, <0 = infinite, 0 = poll

  // Semaphore API
  struct Semaphore *CreateSemaphore(int initialCount, int maxCount);
  void WaitForSemaphore(Semaphore *sem, int timeoutMillis);
  void ReleaseSemaphore(Semaphore *sem, int releaseCount, int *previousCount);

  // Suspend / Resume
  void Suspend();
  void Resume();

  void Wake(); // 唤醒线程（清除 sleep/wait 状态）
...
private:
  ...
  Semaphore *_waitingSemaphore = nullptr; // 当前正在等待的信号量
