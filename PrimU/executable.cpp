	...
	// 先尝试 ELF（保持原逻辑）
	{
		...
		if (reader.load(_path)) {
			...
			for (int i = 0; i < segSize; i++)
			{
				...
				_size = 16 * 1024 * 1024; //seg->get_memory_size();
			...
			//	if (sec->get_type() == 8) {
			//		_address = sec->get_address();
			//		_size = sec->get_size();
			//		printf("Loading bss: VAddr=0x%08X, Size=0x%08X, VEnd=0x%08X\n", _address, _size, _address + _size);
			//		ErrorCode err;
			//		MemoryBlock* memBlock;
			//		__check((err = sMemoryManager->StaticAlloc(_address, _size, &memBlock)), ERROR_OK, err);

			//		RealPtr addr = memBlock->GetRAddr();
			//		if (memset(addr, 0, _size)) {
			//			sMemoryManager->StaticFree(_address);
			//			return ERROR_GENERIC;
			//		}
			//	}
